...
test("Tibber-bestand leest alle vijf tarieven (incl. btw -> excl.)", async () => {
  ...
  const v = Object.fromEntries(Object.entries(rec.tarieven).map(([k, x]) => [k, [x!.waarde, x!.bron]]));
  assert.deepEqual(v, {
...
test("NextEnergy-bestand leest centen en de vaste kosten 'ervoor'", async () => {
  ...
  assert.equal(rec.tarieven.stroomInkoopopslag?.waarde, ex(0.021));
  assert.equal(rec.tarieven.gasInkoopopslag?.waarde, ex(0.079));
  assert.equal(rec.tarieven.stroomVastPerMaand?.waarde, ex(5.99));
  ...
  assert.equal(rec.tarieven.terugleverCorrectie?.waarde, 0);
});
...
test("ANWB-bestand neemt de aangekondigde nieuwe gasprijs, stroomopslag uitgelezen", async () => {
  ...
  assert.equal(rec.tarieven.stroomInkoopopslag?.waarde, ex(0.018));
  assert.equal(rec.tarieven.gasInkoopopslag?.waarde, ex(0.0768));
